...
import { FaCalendarCheck, FaPhone, FaStar, FaUserMd, FaHeart, FaLock, FaClock, FaAward } from 'react-icons/fa';
...
const Hero = () => {
  ...
  return (
    <section id="home" className="relative bg-gradient-to-br from-[#2e76ad] via-[#3a8ccc] to-[#256399] min-h-screen sm:min-h-[90vh] lg:min-h-screen flex items-center overflow-hidden">
      {/* Background Pattern */}
      ...
      <div className="container mx-auto px-4 sm:px-6 relative z-10 py-8 sm:py-12 lg:py-16">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center">
          ...
          <div className="text-white space-y-6 sm:space-y-8">
            {/* Badge */}
            <div className="inline-flex items-center space-x-2 bg-white/20 backdrop-blur-sm px-3 sm:px-4 py-1.5 sm:py-2 rounded-full border border-white/30">
              <FaAward className="text-yellow-300 text-sm sm:text-base" />
              <span className="font-semibold text-xs sm:text-sm md:text-base">Leading Psychiatry Care Since 2005</span>
            </div>

            {/* Main Heading */}
            <h1 className="text-3xl xs:text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-black leading-tight mb-3 sm:mb-4 md:mb-6">
              <span className="block text-white mb-1 sm:mb-2">Your Journey to</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 md:gap-5 py-4 sm:py-5 md:py-6 bg-white/10 rounded-xl sm:rounded-2xl p-4 sm:p-5 md:p-6 backdrop-blur-sm border border-white/20">
              {[
                ...
                <div key={index} className="flex items-center space-x-2 sm:space-x-3 md:space-x-4 bg-white/10 rounded-lg sm:rounded-xl p-2 sm:p-3 hover:bg-white/20 transition-all">
                  <span className="text-2xl sm:text-3xl flex-shrink-0">{feature.icon}</span>
                  <span className="text-blue-50 font-semibold text-sm sm:text-base md:text-lg">{feature.text}</span>
                </div>
              ))}
            </div>

            {/* CTA Buttons */}
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 md:gap-5 pt-4 sm:pt-6 md:pt-8">
              <button 
              ...
              >
                <FaCalendarCheck size={18} className="sm:w-5 sm:h-5 md:w-6 md:h-6 group-hover:rotate-12 transition-transform" />
              ...
              >
                <FaPhone size={16} className="sm:w-4 sm:h-4 md:w-5 md:h-5 group-hover:scale-125 transition-transform" />
            ...
            <div className="flex flex-wrap items-center gap-3 sm:gap-4 md:gap-6 pt-4 sm:pt-5 md:pt-6">
              <div className="flex items-center space-x-2 bg-white/10 px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg">
                ...
                <span className="text-blue-100 text-xs sm:text-sm">Expert Psychiatrists</span>
              </div>
              <div className="flex items-center space-x-2 bg-white/10 px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg">
                <FaLock className="text-green-300 w-4 h-4 sm:w-[18px] sm:h-[18px]" />
                <span className="text-blue-100 text-xs sm:text-sm">Complete Privacy</span>
              </div>
              <div className="flex items-center space-x-2 bg-white/10 px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg">
                <FaClock className="text-green-300 w-4 h-4 sm:w-[18px] sm:h-[18px]" />
                <span className="text-blue-100 text-xs sm:text-sm">Flexible Timings</span>
              </div>
            </div>
          </div>
          ...
          <div className="relative mt-8 lg:mt-0">
            ...
            <div className="bg-white/10 backdrop-blur-md rounded-2xl sm:rounded-3xl p-4 sm:p-5 md:p-6 border border-white/20 shadow-2xl">
              <div className="text-center mb-4 sm:mb-5 md:mb-6">
                <h3 className="text-xl sm:text-2xl font-bold text-white mb-1 sm:mb-2">Our Expert Psychiatrists</h3>
                <p className="text-blue-200 text-sm sm:text-base">Dedicated to your mental wellness</p>
              </div>

              {/* Psychiatrists List */}
              <div className="space-y-3 sm:space-y-4">
                {psychiatrists.map((doctor, index) => (
                  <div key={index} className="bg-white/5 rounded-xl sm:rounded-2xl p-3 sm:p-4 border border-white/10 hover:bg-white/10 transition-all duration-200">
                    <div className="flex items-center space-x-3 sm:space-x-4">
                      <div className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 bg-white rounded-xl sm:rounded-2xl flex items-center justify-center flex-shrink-0">
                        <span className="text-xl sm:text-2xl">{doctor.avatar}</span>
                      </div>
                      <div className="flex-1 min-w-0">
                        <h4 className="text-white font-bold text-sm sm:text-base md:text-lg truncate">{doctor.name}</h4>
                        <p className="text-blue-200 text-xs sm:text-sm truncate">{doctor.specialty}</p>
                        <div className="flex items-center space-x-1 sm:space-x-2 mt-1 flex-wrap">
                          <div className="flex items-center space-x-0.5 sm:space-x-1">
                            {[...Array(5)].map((_, i) => (
                              <FaStar key={i} className="text-yellow-300 w-2.5 h-2.5 sm:w-3 sm:h-3" />
                            ))}
                            <span className="text-white text-xs ml-0.5 sm:ml-1">{doctor.rating}</span>
                          </div>
                          <span className="text-blue-300 text-xs">•</span>
                          <span className="text-blue-200 text-xs">{doctor.experience}</span>
                        </div>
                      </div>
                    </div>
              ...
              <div className="grid grid-cols-3 gap-2 sm:gap-3 md:gap-4 mt-4 sm:mt-5 md:mt-6 pt-3 sm:pt-4 border-t border-white/20">
                <div className="text-center">
                  <div className="text-white font-bold text-sm sm:text-base md:text-lg">5K+</div>
                  <div className="text-blue-200 text-xs">Patients</div>
                </div>
                <div className="text-center">
                  <div className="text-white font-bold text-sm sm:text-base md:text-lg">15+</div>
                  <div className="text-blue-200 text-xs">Years</div>
                </div>
                <div className="text-center">
                  <div className="text-white font-bold text-sm sm:text-base md:text-lg">98%</div>
            ...
            <div className="mt-4 sm:mt-5 md:mt-6 bg-red-500/20 backdrop-blur-sm border border-red-300/30 rounded-xl sm:rounded-2xl p-3 sm:p-4">
              <div className="flex items-center space-x-2 sm:space-x-3">
                <FaHeart className="text-red-300 w-5 h-5 sm:w-6 sm:h-6 flex-shrink-0" />
                <div className="min-w-0">
                  <h4 className="text-white font-bold text-sm sm:text-base">Urgent Support</h4>
                  <p className="text-red-100 text-xs sm:text-sm">Immediate assistance available</p>
                </div>
              </div>

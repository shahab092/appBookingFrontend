...
import { 
  FiHome, FiCalendar, FiFileText, FiPackage, FiMessageSquare, 
  FiDollarSign, FiSettings, FiUsers, FiUserPlus, FiActivity,
  FiShoppingCart, FiDroplet, FiX, FiBarChart, FiShield
} from 'react-icons/fi';

const DynamicSidebar = ({ isSidebarOpen, setIsSidebarOpen, activeItem, setActiveItem, onLogout, userRole }) => {
    ...
    <>
      ...
      >
        ...
        <div className={`p-6 border-b border-gray-200 flex items-center gap-3 ${!isSidebarOpen && 'justify-center'}`}>
          ...
          <h1 className={`text-xl font-bold text-[#2e76ad] tracking-wide whitespace-nowrap overflow-hidden transition-opacity duration-200 ${!isSidebarOpen && 'opacity-0 w-0'}`}>
            Ashfaq Psychiatric<br />Hospital
          </h1>
        </div>
          ...
          <ul className="space-y-1">
            {menuItems.map((item) => (
              <li key={item.id}>
                ...
                >
                  <span className={`text-lg flex-shrink-0 ${activeItem === item.id ? 'text-white' : 'text-blue-600'}`}>
                    {item.icon}
                  </span>
                  <span className={`whitespace-nowrap overflow-hidden transition-opacity duration-200 ${!isSidebarOpen && 'opacity-0 w-0'}`}>
                    {item.label}
                  </span>
                </button>
              </li>
